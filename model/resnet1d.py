...
if __name__ == '__main__':
    from basic_conv1d import *
else:
    from model.basic_conv1d import *
...
class ResNet1d(nn.Sequential):
    ...
    def __init__(self, block, layers, kernel_size=3, num_classes=2, input_channels=3, inplanes=64, fix_feature_dim=True,
                 ...
                 bn_final_head=False, bn_head=True, act_head="relu", concat_pooling=True, use_top=True):
        ...
        if use_top:
            ...
            head = nn.Sequential(
                ...
                Flatten(),
                nn.BatchNorm1d(inplanes * 8),
                nn.Linear(inplanes * 8, inplanes * 8),
                nn.BatchNorm1d(inplanes * 8),
                nn.ReLU(),
                nn.Linear(inplanes * 8, 2),
                # nn.Sigmoid()
            )
...
class WideResNet1d(nn.Sequential):
    def __init__(self, input_channels: int, num_groups: int, N: int, num_classes: int, k: int = 1, drop_p: float = 0.0,
        ...
        for i in range(num_groups):
            layers += _make_group(N, n_channels[i], n_channels[i + 1], BasicBlock1dwrn, (1 if i == 0 else 2), drop_p,
                                  ks=kernel_size)

        # layers += [nn.BatchNorm1d(n_channels[-1]), nn.ReLU(inplace=True), nn.AdaptiveAvgPool1d(1),
        #           Flatten(), nn.Linear(n_channels[-1], num_classes)]
        head = create_head1d(n_channels[-1], nc=num_classes, lin_ftrs=lin_ftrs_head, ps=ps_head, bn_final=bn_final_head,
                             bn=bn_head, act=act_head, concat_pooling=concat_pooling)
        layers.append(head)
...
if __name__ == '__main__':
    ...
    model = resnet1d50(num_classes=5, input_channels=1, inplanes=128, use_top=True)

    summary(model, (1, 2560), -1, 'cpu')
