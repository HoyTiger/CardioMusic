...
train_dataset = DREAMERDataset(ecg_dataset.Xtrain, ecg_dataset.ytrain)
train_data_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)

test_dataset = DREAMERDataset(ecg_dataset.Xtest, ecg_dataset.ytest)
test_data_loader = DataLoader(test_dataset, batch_size=64, shuffle=True)

model = resnet1d50(num_classes=2, input_channels=1, inplanes=128, use_top=True).to(device)
# model_dict = model.state_dict()
# pretrain = torch.load('ecg_128.pth')
...
for epoch in range(0, epochs):
    ...
    with torch.no_grad():
        ...
        scheduler.step(loss)
        te_mse_loss.append(loss.item())

        if loss < min_loss:
            torch.save(model.state_dict(), 'ecg_128.pth')
            min_loss = loss
            es = 0
        else:
            es += 1
...
# plt.plot(tr_mse_loss, label='train')
plt.plot(te_mse_loss, label='test')
plt.legend()
plt.show()