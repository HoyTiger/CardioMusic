...
class ResNet1D(nn.Module):
    ...
    def __init__(self, in_channels=1, base_filters=64, kernel_size=3, stride=2, groups=64, n_block=16, n_classes=4,
                 downsample_gap=2,
        ...
        self.dense_latent = nn.Linear(out_channels,n_classes)
...
if __name__ == '__main__':
    ...
    epochs = 100
    dataset = DEAM()
    train_dataset = DEAMDataset(dataset.Xtrain, dataset.ytrain)
    test_dataset = DEAMDataset(dataset.Xtest, dataset.ytest)
    train_data_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)
    test_data_loader = DataLoader(test_dataset, batch_size=64, shuffle=True)

    model = ResNet1D(in_channels=18,n_classes=4).to(device)
